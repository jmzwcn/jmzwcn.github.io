...
从Service本身看，有三种方式来暴露访问：<br/>
    ...
    ClusterIP：使用集群内的私有ip —— 这是默认值<br/>

    NodePort：除了使用cluster ip外，也将service的port映射到每个node的一个指定内部port上，映射的每个node的内部port都一样。<br/>

    LoadBalancer：使用一个ClusterIP & NodePort，但是会向cloud provider申请映射到service本身的负载均衡。<br/>
