...
```
其实观察上面的使用方式，我们已经看出`KUBE_MASTER`这个参数，是单点的，也就是HA里需重点处理的，所以k8s HA各种方案，其实都是围绕这个展开的。
